...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, id):
        ...
        """
        values = [float(v) if v not in ['<pad>', 'NA', 'NA\n'] else 2 for v in values_li]

        return words, x, is_main_piece, tags, y, seqlen, values, 2
...
def load_dataset():
    ...
    for split in ['train', 'dev', 'test']:
        ...
        filename = 'train_100' if split == 'train' else split
        ...
        with open('./data'+'/'+filename+'.txt') as f:
            ...
            for i, line in enumerate(lines):
                ...
                if i != 0 and split_line[0] != "<file>":
                    ...
                    tag_prominence = rediscretize_tag(value_prominance, 3)
